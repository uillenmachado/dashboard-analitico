  ...
  async loadExcelFile(file) {
    try {
      ...
      if (!workbook.Sheets[sheetName]) {
        ...
        console.log(
          `⚠️ Aba "Notas Fiscais" não encontrada, usando: ${sheetName}`,
        );
      }
    ...
    const processedData = rawData
      .map((row, index) => {
        try {
          ...
          processedRow.dataPagamento = this.parseDate(
            processedRow.dataPagamento,
          );
          processedRow.previsaoRecebimento = this.parseDate(
            processedRow.previsaoRecebimento,
          );

          // Process numeric values
          processedRow.valorNota = this.parseNumber(processedRow.valorNota);
          processedRow.iss = this.parseNumber(processedRow.iss);
          processedRow.valorLiquido = this.parseNumber(
            processedRow.valorLiquido,
          );
          processedRow.diasPagamento = this.parseNumber(
            processedRow.diasPagamento,
          );

          // Calculate additional fields
          processedRow.diasEmAberto = this.calculateDaysOpen(processedRow);
          processedRow.agingBucket = this.calculateAgingBucket(
            processedRow.diasEmAberto,
          );
          ...
          processedRow.statusConciliado = this.cleanText(
            processedRow.statusConciliado,
          );
          processedRow.recebido = this.cleanText(processedRow.recebido);
          processedRow.statusPagamento = this.cleanText(
            processedRow.statusPagamento,
          );
